        ...
        public static void HandleObjectUseMessage(WorldClient client, ObjectUseMessage message)
        {
            ...
            switch (item.Template.Id)
            {
                case 12332: //add level < 25
                    if (client.Character.Level < 150)
                    {
                        client.Character.LevelUp(5);
                ...
                case 684://add kolichas
                         ...
                         //pruebas
                    client.Send(new ClientUIOpenedMessage(2)); // es mas, no se si sirve xD 
                    client.Send(new ClientUIOpenedByObjectMessage(2, (uint)item.Guid));
                    client.Character.Inventory.RemoveItem(item, 1);
                    break;
                case 21135://Emote Aleatorio
                    var rnd = new Random();
                    int  emoternd = rnd.Next(2,216);
                    

                   
                   var emoteGenerado = ChatManager.Instance.GetEmote(emoternd);

                    client.Character.AddEmote(emoteGenerado.EmoteId);
