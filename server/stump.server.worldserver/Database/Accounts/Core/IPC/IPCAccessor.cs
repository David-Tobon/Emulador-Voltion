...
namespace Stump.Server.WorldServer.Core.IPC
{
    public class IPCAccessor : IPCEntity
    {
        ...
        public IPCAccessor()
        {
            TaskPool = new SelfRunningTaskPool(TaskPoolInterval, "IPCAccessor Bloque de Tareas");
        ...
        {
            logger.Info("IPC Conexion establecida");
        ...
        private void OnClientDisconnected()
        {
            ...
            logger.Info("IPC conexion perdida");

            foreach (var request in Requests.Values)
            {
                request.Cancel("world server - desconectado");
            }
        ...
        private void OnAccessGranted(CommonOKMessage msg)
        {
            ...
            logger.Info("Accesso Auth, Acceso Garantizado");
#if DEBUG
        ...
        private void OnAccessDenied(IIPCErrorMessage error)
        {
            ...
            AccessGranted = false;
            logger.Error("Accesso Auth. Acceso denegado ! Razon : {0}", error.Message);
        ...
        private void Tick()
        {
            ...
            if (!IsReacheable)
            {
                ...
                {
                    logger.Error("Conexion con  {0}:{1} Fallo. Intenando nuevamente en {2}s", RemoteHost, RemotePort,
                        UpdateInterval/1000);
                    return;
                }
        ...
        private void ProcessReceive(object sender, SocketAsyncEventArgs e)
        {
            ...
            {
                logger.Error("Desconexion Forzada durante la Recepcion : " + ex);
