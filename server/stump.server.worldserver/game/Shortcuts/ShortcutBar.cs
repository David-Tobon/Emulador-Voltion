...
namespace Stump.Server.WorldServer.Game.Shortcuts
{
    public class ShortcutBar
    {
        ...
        public SpellsPreset GetNetworkSpellsPreset() => new SpellsPreset(0, SpellsShortcuts.Select(x => (ushort)x.Value.SpellId).ToList());
        ...
        public void AddSpellShortcut(int slot, short spellId, bool Custom = false)
        {
            var spell = Owner.Spells.GetSpell(spellId);
            if (spell.Record.SpellId == spellId && spell.Record.Active != true)
                return;
