    ...
    public class LoterieSkill : CustomSkill
    {
        ...
        public override int StartExecute(Character character)
        {
            if (character.WorldAccount.LastLoterieDate != null && character.WorldAccount.LastLoterieDate.Date == DateTime.Now.Date)
            {
                ...
                if (ticket != null)
                {
                    ...
                    character.SendServerMessage("Acabas de usar un Billete de Loteria, ¡Buena Suerte!", Color.YellowGreen);
                    GetPrice(character);
                }
                else
                    character.OpenPopup("La loteria es gratuita una vez al dia, Sí quiere jugar de nuevo, espere hasta mañana o compre un Billete de Loteria.");
            }
            else
            {
                GetPrice(character);
        ...
        private void GetPrice(Character character)
        {
            ...
            if (Sort != null)
            {
                ...
                character.DisplayNotification("¡Has ganado un Nuevo Objeto! Descubrelo en tu inventario.");
            }
            else
            {
                character.OpenPopup("Lo sentimos pero esta vez no ganó nada, intentalo de nuevo. ...");
            }
        }
    }
}
