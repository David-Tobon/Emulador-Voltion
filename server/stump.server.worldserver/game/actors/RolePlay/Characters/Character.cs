/* 
 * 
 *  XD Lo cambie complamente por el de otro emulador desbuggeado, pero dejé el original aqui para que no se me perdiera: Att David Tobon <3
 * 
 * 
 * 
 * 
...
namespace Stump.Server.WorldServer.Game.Actors.RolePlay.Characters
{
    public sealed class Character : Humanoid, IStatsOwner, IInventoryOwner, ICommandsUser
    {
        ...
        void OnLoggedIn()
        {
            ...
            {
                var item = Inventory.TryGetItem(ItemManager.Instance.TryGetTemplate(30000));
                if (!WorldAccount.Vip && !this.Inventory.HasItem(ItemManager.Instance.TryGetTemplate(30000)))
                {
                    item = this.Inventory.AddItem(ItemManager.Instance.TryGetTemplate(30000), 1);
                ...
                item = Inventory.TryGetItem(ItemManager.Instance.TryGetTemplate(30000));
                if (!this.Inventory.HasItem(ItemManager.Instance.TryGetTemplate(30000)))
                {
                    item = this.Inventory.AddItem(ItemManager.Instance.TryGetTemplate(30000), 1);
                    item.Effects.Clear();
                    item.Invalidate();
                ...
                StartQuest(1042); //Inicia con la mision Tal xd
        ...
        public void ChangeAlignementSide(AlignmentSideEnum side)
        {
            ...
            {
                SendServerMessage("Felicidades, ahora eres <b>Brakmariano</b> !");
            }
        }
        ...
        public bool IncrementPrestige()
        {
            ...
            OpenPopup($"Felicidades, Has Obtenido el Prestigio  {PrestigeRank} ! \r\n¡ Ahora eres nivel 1! \r\n Sientes un poder recorriento todo tu ser, ¡Ahora eres un poco más Fuerte que Antes!");
    ...
    public sealed class Character : Humanoid, IStatsOwner, IInventoryOwner, ICommandsUser
    {
        ...
        void OnLoggedIn()
        {
            ...
            if (IsFirstConnection)
            {
                ...
                if (UserGroup.Id == 2)
                {
                    ...
                    SendServerMessage("Tipo de cuenta: <b>★[VIP] </b>", Color.LightGoldenrodYellow);
                    World.Instance.SendAnnounce("El miembro VIP: <b>" + Name + "</b> se ha creado una Cuenta", Color.Gold);
                }
                if (UserGroup.Id >= 3)
                {
                    SendServerMessage("Tipo de cuenta: <b> [STAFF] </b>", Color.LightGoldenrodYellow);
                    World.Instance.SendAnnounce("El Administrador: <b>" + Name + "</b> se ha creado una Cuenta", Color.Red);
                }
                if (UserGroup.Id == 2)
                {
                    SendServerMessage("Tipo de cuenta: <b>★[VIP] </b>", Color.LightGoldenrodYellow);
                ...
                if (Sex == SexTypeEnum.SEX_MALE)
                    World.Instance.SendAnnounce("Bienvenido a Dofus Voltion <b>" + Name + "</b> quien acaba de crear su personaje con la Raza:  <b>" + name + "</b> ¡Feliz Aventura!.", Color.White);
                else
                    World.Instance.SendAnnounce("Bienvenida a Dofus Voltion <b>" + Name + "</b> quien acaba de crear su personaje con la Raza:  <b>" + name + "</b> ¡Feliz Aventura!.", Color.White);
            ...
            if (!IsInFight() && (DateTime.Now - (DateTime)Account.LastConnection).TotalSeconds >= 10800) // 3 Horas
            {
                Client.Send(new PopupWarningMessage(5, "Voltion", "Hola y Bienvenido a <b>Dofus Voltion</b>, el servidor se encuentra en constante desarrollo para brindar una mejor experiencia de juego. " +
                    "Recuerda que aunque las donaciones no son obligatorias, nos ayudan a mejorar y a mantenernos dia a dia. <br />¡Todo el equipo de Dofus Voltion te desea una Feliz Aventura!"));
            }
            ...
            if (DateTime.Now.IsWeekEnd())
            {
                if (!IsInFight() && (DateTime.Now - (DateTime)Account.LastConnection).TotalSeconds >= 10800) // 3 Horas
                {
                    Client.Character.DisplayNotification("Recuerda que el Fin de Semana hay rates de Experiencia y Botín Aumentados. ", NotificationEnum.INFORMATION);
                }
            }

            if (DateTime.Now.IsWeekDay())
            {
                if (!IsInFight() && (DateTime.Now - (DateTime)Account.LastConnection).Seconds >= 10800) // 3 horas
                {
                    Client.Character.DisplayNotification("Recuerda jugar a la loteria diariamente en tu Merkasaco. ", NotificationEnum.INFORMATION);
                }
            }
        ...
        public override string Name
        {
            get
            {
                    ...
                    if ((RoleEnum)Account.UserGroupId == RoleEnum.Moderator) {

                    return (RoleEnum)Account.UserGroupId >= RoleEnum.Moderator ? $"[VIP]{m_record.Name}": m_record.Name;
        ...
        */
        ...
        public string Namedefault
        {
            get
            {
                return (RoleEnum)Account.UserGroupId >= RoleEnum.Moderator ?
                    ((RoleEnum)Account.UserGroupId == RoleEnum.Moderator ? m_record.Name : $"[{m_record.Name}]") :
                    m_record.Name;
            }
        }
        public override int Id
        ...
        {
            MuteUntil = null;
            SendServerMessage("Ha sido despedido.", Color.Red);
        }
        ...
        public bool IncrementPrestige()
        {
            ...
            AddTitle(PrestigeManager.Instance.GetPrestigeTitle(PrestigeRank));
             ...
             #region Regalos al Subir de Prestigio
            switch (PrestigeRank)  // Regalos a Dar por Prestigios 
             {

                case 1:   break;
                case 2: AddTitle(123);  break;
                case 3: AddOrnament(111); AddTitle(127);  break;    
                case 4: break;
                case 5: break;
                case 6: break;    
                case 7: break;
            ...
            OpenPopup($"¡ Ahora Eres Prestigio  {PrestigeRank} ! \r\n¡ Regresas al Nivel 1 ! \r\n ¡Ahora eres un poco más poderoso!");
            //Mensaje a todos
             World.Instance.SendAnnounce("Felicidades al  Jugador " + Name + ". Ahora es Prestigio [ " +PrestigeRank +" ] Es un poco más poderoso que antes.", Color.White);
        ...
        public bool DecrementPrestige()
        {
            ...
            OpenPopup(
                string.Format(
                    "Acabas de subir al rango de prestigio{0}. Vuelve al nivel 1 y ha adquirido bonificaciones permanentes visibles en el objeto. '{1}' de tu inventario, ",
                    PrestigeRank + 1, item.Template.Name) +
                "las bonificaciones se aplican sin equipar el objeto. Debes volver a conectarte para actualizar tu nivel.");
        ...
        public int ComputeWonArenaCaliston()
        {
            ...
            if (this.Level >= 51 && this.Level <= 149)
            {
                return (20);
            }
            if (this.Level >= 150)
